...
from utils import AverageMeter, Metric, SegWithAuxLoss, Checkpoint, get_lr, count_parameters, PolyLR
from models.utils import set_bn_momentum
...
def get_args():
    ...
    parser.add_argument("--backbone", default="repvgg", type=str,
...
def resume(model, optimizer, scaler, scheduler):
    if os.path.isfile("./best.pth"):
        ...
        best_acc = loaded["acc"]
        print(f"loaded all parameters from best checkpoint.")
        print()
...
def train_one_epoch(model, loader, criterion, metric, optimizer, scaler, epoch):
    model.train()
    ...
    for batch_idx, (inputs, targets) in enumerate(loop):
        ...
        loop.set_description(f"Train -> Epoch:{epoch} Loss:{loss_total.avg:.4} Metric:{metric_total.avg:.4},"
                             ...
                             f" LR: {get_lr(optimizer) * 10}, Params: {count_parameters(model)}")
...
def main():
    ...
    train_ds = CityScapes(phase="train", mean=backbone.IMAGENET_MEAN, std=backbone.IMAGENET_STD)
    valid_ds = CityScapes(phase="val", mean=backbone.IMAGENET_MEAN, std=backbone.IMAGENET_STD)
